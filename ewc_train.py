...
from mp.models.ewc.ewc import EWC
...
for run_ix in range(config['nr_runs']):
    ...
    model = EWC(input_shape=config['input_shape'], nr_labels=nr_labels,
    ...
    agent = EWCAgent(model=model, label_names=label_names, device=config['device'])
