...
class EWCAgent(SegmentationAgent):
    ...
    def train(self, results, loss_f, train_dataloader, test_dataloader, config,
        ...
        rtpt = RTPT(name_initials='MM', experiment_name='Bottleneck', max_iterations=nr_epochs)
        rtpt.start()
        ...
        fisher, mean = self.calc_fisher_mean(train_dataloader, loss_f)
        self.model.update_fisher_mean(fisher, mean)
        self.model.finish()
    ...
    def perform_training_epoch(self, loss_f, train_dataloader, config,
        ...
        for data in tqdm(train_dataloader, disable=True):
            ...
            if loss_seg.is_cuda:
                loss_ewc = torch.zeros(1).to(loss_seg.get_device())
            else:
                loss_ewc = torch.zeros(1)

            if self.model.fisher != None and not config['unet_only']:
                for name, param in self.model.unet.named_parameters():
                    loss_ewc += (self.model.fisher[name] * (param - self.model.mean[name]) ** 2).sum()
            ...
            acc.add('loss_ewc', float(loss_ewc.detach().cpu()), count=len(inputs))

        # self.model.unet_scheduler.step()

        if print_run_loss:
            print('\nrunning loss: {} - ewc: {} - time/epoch {}'.format(acc.mean('loss'), acc.mean('loss_ewc'), round(time.time()-start_time, 4)))
    ...
    def calc_fisher_mean(self, dataloader, loss_f):
        ...
        for input in dataloader:
            model.zero_grad()
            input, targets = self.get_inputs_targets(input)
            output = self.get_outputs(input)
        ...
        return precision_matrices, mean
